...
export default function Board() {
  ...
  const [showWhiteBoard, setShowWhiteBoard] = useState(false); // State to control visibility of WhiteBoard
  const [chat, setChat] = useState(false);

  const handleWhiteboardToggle = () => {
    setShowWhiteBoard((prev) => !prev);  // Toggle WhiteBoard visibility
  };
  const handleChatbotToggle = () => {
    setChat((prev) => !prev);  // Toggle Chat visibility
  };
  ...
  useEffect(() => {
    if (!_.isEmpty(currBoard)) {
      ...
      document.title = `${currBoard.name} | Hack On`
    }
  }, [currBoard])
          ...
          {editable ? (
            ...
            <BoardHeader
              ...
              style={{
                ...
                boxShadow: '0 2px 5px rgba(0,0,0,0.1)' // Optional: Add shadow for emphasis
              }}
            />
